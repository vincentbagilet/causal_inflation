...
data %>%
  ...
  ggtitle("Distribution of the True Propensity Scores of Units by Treatment Groups") +
...
data %>%
  mutate(treatment = ifelse(treatment == 1, "Treated", "Control")) %>%
  ggplot(., aes(x = y_0, fill = treatment)) +
  geom_density(colour = NA) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  ggtitle("Distribution of the Potential Outcomes Y(0) of Units by Treatment Groups") +
...
With an outcome regression analysis, the average of estimates is equal to `r round(mean(data_simulations$estimate), 0)`! The outcome regression analysis suffers from extrapolation bias.
...
data_simulations <- tibble(sim_id = 1:300) %>%
  ...
  # generate caliper
  crossing(caliper = seq(0.01, 1, 0.01)) %>%
...
summary_sim_matching %>% 
  pivot_longer(cols = c(exag_ratio, bias_all), names_to = "measure") %>% 
  mutate(
    measure = ifelse(measure == "exag_ratio", "Significant", "All")) %>% 
...
ggsave("main_graph_matching_paper.pdf", path = "images", width = 7.4, height = 4.4)
```
