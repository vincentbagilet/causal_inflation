...
Note that, for readability, in this document, we only display the chunks of code that may be important to understand the assumptions behind our simulations and the way we built our simutaion. We do not display all the arguably "technical" code, in particular the one used to generate tables and graphs. All this code is however openly available on the GitHub of the project.
...
ex_data <- baseline_parameters_IV %>% 
...
fixed_parameters <- baseline_parameters_IV #%>% rbind(...)
...
```{r bined_fstat, echo=FALSE, fig.asp=0.8}
simulations_IV %>% 
  filter(model == "IV") %>%
  mutate(
    significant = (p_value <= 0.05),
...
fixed_parameters <- simple_parameters_IV #%>% rbind(...)
vect_iv_strength <- c(seq(0.05, 0.4, 0.05), seq(0.4, 0.6, 0.1))
# vect_iv_strength <- c(0.1)
...
# tic()
# simulations_IV_simple <- pmap_dfr(param_IV_simple, compute_sim_IV)
# beep()
# toc()
# 
# saveRDS(simulations_IV_simple, here("Outputs/simulations_IV_simple.RDS"))
simulations_IV_simple <- readRDS(here("Outputs/simulations_IV_simple.RDS"))

simulations_IV_simple %>% 
