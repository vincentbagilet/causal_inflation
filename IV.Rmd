...
In the case of the IV, the confounding/type M trade-off is  mediated by the 'strength' of the instrument considered. When the instrument only explains a limited portion of the variation in the explanatory variable, the IV can still be successful in avoiding confounders but power can low, potentially leading to high rates of type M error. 
...
- We calibrate the distribution parameters to fit a mix if information from table 1 from both Fujiwara et al. and Cooperman (converting the rainfall into centimeters):
  ...
  - [Fowler (2013)](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=1816649) for an extremely large shock in voter turnout, compulsory voting, finds "that the policy increased voter turnout by 24 percentage points which in turn increased the vote shares and seat shares of the Labor Party by 7 to 10 percentage points." This correspond to a decrease in Republican vote share of approximately 0.3-0.4 when turnout increases by 1% (considering that his result is causal). We thus define the magnitude of the treatment effect $\\beta$ to be of this order of magnitude, even though it might be much smaller as the shock was particularly large. As underlined by Fowler, there is no consensus in the literature on the magnitude of the effect of turnout on vote share.
  ...
  - We set the standard deviation of the omitted variable bias to be of the order of magnitude of the error terms
...
vect_iv_strength <- c(seq(0.05, 0.4, 0.05), seq(0.4, 0.8, 0.1))
# vect_iv_strength <- c(0.1)
vect_ovb_intensity <- c(-1)
n_iter <- 1000

param_IV <- fixed_parameters %>% 
  select(-iv_strength, -ovb_intensity) %>% 
...
sim_IV %>% 
  filter(between(estimate, -1.5, 0.5)) %>%
...
sim_IV %>%
  filter(between(estimate, -2.5, 1.5)) %>%
  filter(model == "IV") %>%
  filter(ovb_intensity == sample(vect_ovb_intensity, 1)) %>%
  ggplot() +
...
summary_sim_IV %>% 
  ...
  labs(
    ...
    color = "Model",
    # title = "Evolution of bias with intensity of the IV",
    # caption = "Only statistically significant estimates"
  ) +
