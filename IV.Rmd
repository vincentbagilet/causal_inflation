...
- We assume that $\delta = \eta$ for simplicity. There is no actual basis for that and we may change that in the future.
...
simple_parameters_IV %>% 
  mutate(N = 10, iv_strength = -0.5, ovb_intensity = 0.5) %>% 
...
ex_data <- simple_parameters_IV %>% 
  mutate(N = 10000, iv_strength = -0.5, ovb_intensity = 0.5) %>% 
