...
We want to vary two parameters only: the bandwidth and the intensity of the omitted variable bias. The function `estimate_rdd` therefore only takes two parameters. The bandwidth is defined as a proportion of the standard deviation of the variable X.
...
compute_true_effect_rdd <- function(data) {
  ...
  return(mean(treated_data$Y1 - treated_data$Y0))
}  

estimate_rdd <- function(data, bw = 0.5) {
  data_in_bw <- data %>% 
    filter(dplyr::between(x, mean(x) - bw/2*sd(x), mean(x) + bw/2*sd(x)))
...
```{r}
compute_simulation_rdd <- function(ovb_intensity = 10, bw = 0.5) {
  generate_data_rdd(ovb_intensity = ovb_intensity) %>% 
    estimate_rdd(bw = bw) %>% 
    mutate(
      bw = bw,
