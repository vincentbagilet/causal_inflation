...
- We assumes that grades are SAT scores. One can think of the qualifying exam as a state level exam that gives the opportunity to students with lower grades to receive additional lessons. For simplicity, we assume that the **scores have a mean of 1060 and standard error of 200** (based on [statistics provided by the Department of Education](https://nces.ed.gov/programs/digest/d20/tables/dt20_226.40.asp)). Since these scores depend on the unobserved ability, we adjust the values of the parameters to get the appropriate distributions.
...
- We assume that the unobserved ability has a standard deviation 
- We consider that the threshold is located at the center of the distribution ($q_c = 0.5$). The bandwidth is symmetric around the cutoff. Considering a cutoff in the middle of the distribution enables us to consider very wide bandwidths, up to including the whole sample in the analysis
...
baseline_param_RDD <- tibble(
  ...
  sigma_u = 3,
  mu_h = 1050,
  sigma_h = 170, #sqrt(sd(qual)^2 - sd(u)^2) = sqrt(200^2 - sd(u)^2)
  sigma_e = 100,
  alpha = 550,
  beta = 6,
  gamma = 0.5,
...
Then, we quickly look at the distributions of the different variables.

```{r}
ex_data_RDD %>% 
  pivot_longer(
    cols = c(final0, final1),
    names_to = "potential_outcome",
    values_to = "potential_final"
  ) %>% 
  mutate(potential_outcome = factor(str_remove(potential_outcome, "final"))) %>% 
  ggplot(aes(x = potential_final, fill = potential_outcome, color = potential_outcome)) +
  geom_density() +
  labs(
    x = "Final score",
    y = "Density",
    ...
    title = "Distribution of potential final scores"
  )

ex_data_RDD %>% 
  ggplot(aes(x = u, fill = treated, color = treated)) +
  geom_histogram() + 
  labs(
    title = "Distribution of ability",
    x = "Ability (u)",
    y = "Density"
  ) 

ex_data_RDD %>% 
  ggplot(aes(x = qual, fill = treated, color = treated)) +
  geom_histogram() + 
  labs(
...
ex_data_RDD %>% 
  ggplot(aes(x = final, fill = treated, color = treated)) +
  geom_histogram() + 
  labs(
...
We then look at the relations between the variables:

```{r}
ex_data_RDD %>% 
  define_bw(0.5) %>%
  ggplot(aes(x = final, y = u, color = treated_bw)) + 
  geom_point()
