...
description: "In this document, we run a simulation exercise to illustrate the existence of a trade-off between Omitted Variable Bias (OVB) and type M error in the context of a Regression Discontinuity Design (RDD). This trade-off is mediated by the size of the bandwidth considered in the analysis. The underlying idea is that the smaller the bandwidth, the more comparable units are and therefore the smaller the risk of OVB is. Yet, with a smaller bandwidth, sample size and thus power decrease, increasing type M error."
author:
...
include_graphics(here("images/DAG_RDD.png"))
```
...
baseline_parameters_RDD %>% 
  ...
  labs(
    ...
    subtitle = "Illustration of the definition of the bandwidth"
  )
```
...
```{r example_output_simulation_RDD, echo=FALSE}
baseline_parameters_RDD %>% 
  mutate(delta = 1, bw = 0.1) %>% 
  pmap_dfr(compute_simulation_rdd)
```
...
simulations_rdd %>% 
  ...
  geom_histogram() +
  geom_vline(aes(xintercept = mean(abs(estimate[significant == "Significant"])))) +
  ...
  labs(
    ...
    caption = "The solid line represents the average mean effect 
      for significant estimates (in absolute value), 
